...
# 언어에 따라 코드를 파싱하는 함수
def parse_code_by_language(file_path, language, identifier):

    temp_path = f'/home/lbs/codedb/temp/{identifier}.txt'
    # 언어에 해당하는 쿼리 파일 경로 및 실행 명령어 설정
    query_file_path = QUERY_FILE[language]
    parse_command = f'{CUSTOM_TREE_SITTER} query {query_file_path} {file_path} > {temp_path}'
